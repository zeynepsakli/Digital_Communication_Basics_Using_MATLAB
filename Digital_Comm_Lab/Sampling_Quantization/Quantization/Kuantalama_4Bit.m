...
subplot(421)
stem(nx,xq);
title('20 Hz için 4 bit kuantalanmýþ iþaret')
...
title('60 Hz için 4 bit kuantalanmýþ iþaret')
hata=pam-xq;   %hatayý hesaplatan kýsým
...
title('100 Hz için 4 bit kuantalanmýþ iþaret')
hata=pam-xq;   %hatayý hesaplatan kýsým
...
title('200 Hz için 4 bit kuantalanmýþ iþaret')
hata=pam-xq;   %hatayý hesaplatan kýsým
ortalama_hata=sum(hata.^2)./length(hata);   %ortalama kuantalama hatasý
SNR_hesaplat=10*log10(var(pam)/var(hata)) %snr hesabý(Ps/P(n,q))
